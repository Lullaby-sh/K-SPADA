...
def train(adata, gmt_path, project=None,pre_weights='', label_name='Celltype',max_g=300,max_gs=300,mask_ratio =0.015, n_unannotated = 1,batch_size=8, embed_dim=48,depth=2,num_heads=4,lr=0.001, epochs= 10, lrf=0.01, use_gcn=False, data_batch=[], **kwargs):
    r"""
    ...
    fit_model(adata, gmt_path, project=project,pre_weights=pre_weights, label_name=label_name,
              ...
              embed_dim=embed_dim,depth=depth,num_heads=num_heads,lr=lr, epochs= epochs, lrf=lrf, use_gcn=use_gcn, data_batch=data_batch, **kwargs)


def pre(adata,model_weight_path,project,laten=False,save_att = 'X_att', save_lantent = 'X_lat',n_step=10000,cutoff=0.1,n_unannotated = 1,batch_size=50,embed_dim=48,depth=2,num_heads=4, use_gcn=False, data_batch=[]):
    r"""
    ...
    adata = prediect(adata,model_weight_path,project=project,mask_path = mask_path,laten=laten,
             save_att = save_att, save_lantent = save_lantent,n_step=n_step,cutoff=cutoff,n_unannotated = n_unannotated,batch_size=batch_size,embed_dim=embed_dim,depth=depth,num_heads=num_heads, use_gcn=use_gcn, data_batch=data_batch)
    return(adata)
