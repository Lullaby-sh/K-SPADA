...
def fit_model(adata, gmt_path, project = None, pre_weights='', label_name='Celltype',max_g=300,max_gs=300, mask_ratio = 0.015,n_unannotated = 1,batch_size=4, embed_dim=48,depth=2,num_heads=4,lr=0.001, epochs= 10, lrf=0.01):
    GLOBAL_SEED = 1
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for step, data in enumerate(data_loader):
                ...
                loss = loss_function(pred, labels.to(device)).item()
                accu_loss += loss
                data_loader.desc = "[valid epoch {}] loss: {:.3f}, acc: {:.3f}".format(epoch,
                                                                                       accu_loss.item() / (step + 1),
                                                                                       accu_num.item() / sample_num)
                del exp, labels, pred, loss, data
                torch.cuda.empty_cache()
